...
import { Droplets, Heart, HeartHandshake, Leaf } from "lucide-react";
import { clearRiskScreenerFromSession, readRiskScreenerFromSession } from "@/lib/risk-screener";
        ...
        {!streaming && numSources !== undefined && numSources > 0 && (
          <div className="flex items-center gap-1.5 mb-3">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--sidebar-active)] border border-[var(--sidebar-active-border)] text-[11px] font-semibold text-[var(--primary)]">
              ...
              {numSources} source{numSources !== 1 ? "s" : ""} cited below
            </span>
          </div>
        )}
...
export default function ChatPageClient() {
  ...
  /** Injected into every chat request so answers can be tailored to the risk screener. */
  const [riskScreenerContext, setRiskScreenerContext] = useState<string | null>(null);
  const [riskScreenerAt, setRiskScreenerAt] = useState<string | null>(null);
  ...
  // Load risk screener results (written when the user leaves /assess via "Ask questions…")
  useEffect(() => {
    const s = readRiskScreenerFromSession();
    if (s) {
      setRiskScreenerContext(s.context);
      setRiskScreenerAt(s.at);
    }
  }, []);
  ...
  const handleSubmit = useCallback(
    async (question: string) => {
      ...
      const streamOpts =
        riskScreenerContext && riskScreenerContext.trim()
          ? { userContext: riskScreenerContext }
          : undefined;
      ...
      try {
        for await (const event of streamChat(question, messages, streamOpts)) {
          if (event.type === "token") {
    ...
    [messages, isStreaming, createChat, updateChat, riskScreenerContext]
  );
  ...
  return (
    <div className="flex h-[calc(100vh-56px)]">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        {riskScreenerContext && (
          <div className="flex-shrink-0 border-b border-[var(--card-border)] bg-[var(--sidebar-active)] px-4 py-2.5">
            <div className="max-w-4xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <p className="text-xs text-[var(--foreground)] leading-relaxed">
                <span className="font-semibold text-[var(--primary)]">Personalized mode:</span> your Risk Assessment
                summary is sent with each question to this chat and used for retrieval. Still education-only, not
                medical advice
                {riskScreenerAt
                  ? ` (saved ${new Date(riskScreenerAt).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}).`
                  : "."}
              </p>
              <button
                type="button"
                onClick={() => {
                  clearRiskScreenerFromSession();
                  setRiskScreenerContext(null);
                  setRiskScreenerAt(null);
                }}
                className="text-xs font-medium text-[var(--primary)] border border-[var(--sidebar-active-border)] rounded-lg px-2.5 py-1 hover:bg-white/80 whitespace-nowrap self-start"
              >
                Remove from chat
              </button>
            </div>
          ...
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-full px-6 gap-8 max-w-3xl mx-auto py-8">
              <div className="text-center">
                ...
                <h1 className="font-display mb-2 text-2xl tracking-tight text-[var(--foreground)] md:text-3xl">
                  Research Q&amp;A
                </h1>
                <p className="mx-auto max-w-md text-sm leading-relaxed text-[var(--muted-foreground)]">
                  Concise answers anchored to retrieved literature—numbered citations trace each claim to its source.
                  Educational reference only; not a substitute for individualized medical counsel.
                </p>
              </div>
                ...
                {CATEGORIES.map((cat) => (
                  <div key={cat.label} className="bg-white border border-[var(--card-border)] rounded-2xl p-4 shadow-sm">
                    <div className="mb-3 flex items-center gap-2">
                      <cat.Icon className="h-4 w-4 shrink-0 text-[var(--primary)]" strokeWidth={1.75} aria-hidden />
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex gap-2 items-end rounded-2xl border border-[var(--card-border)] bg-[var(--muted)] px-4 py-2 focus-within:border-[var(--primary)] focus-within:ring-2 focus-within:ring-[var(--primary)]/20 transition-all">
              <textarea
                ...
                placeholder="Ask about a condition, drug, or number you have seen…"
                rows={1}
